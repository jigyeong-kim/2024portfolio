<%@ page contentType="text/html;charset=utf-8"%><%
	...
	upload.initParameter(request, response); // Parameter를 먼저 Set을 해야 upload.Process 전에 얻을 수 있습니다.
	
	// name1 의 이름을 가진 formdata를 받음.
	String[] formData1 = upload.GetRequestValue("name1");
	if (formData1 != null && formData1.length > 0) {
		upload_formDataValue1 = formData1[0]; 
	}
	
	// name2 의 이름을 가진 formdata를 받음.
	String[] formData2 = upload.GetRequestValue("name2");
	...
	event.addUploadBeforeInitializeEventListenerEx(new Raonwiz.Dext5.UploadBeforeInitializeEventListenerEx() {
		public void UploadBeforeInitializeEventEx(Raonwiz.Dext5.Process.Entity.UploadEventEntity parameterEntity) {
			...
			/********** 업로드 시 클라이언트에서 AddFormaData로 추가한 값을 이벤트 함수 안에서 받는 부분 입니다. **********/
			String upload_formDataValue1 = "";
			String upload_formDataValue2 = "";
	...
	String result = "";
    try {
		result = upload.Process(request, response, event); // 서버 이벤트 사용시 반드시 3번째 파라미터의 event 객체를 Setting 해줘야 합니다.
	} catch (Exception e) {
		e.printStackTrace();
	}
