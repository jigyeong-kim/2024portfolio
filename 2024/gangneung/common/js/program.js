    ...
    Alert = function(options) {
        function closeAlert(e) {
            if ("floating" === opt.container && opt.floatingAnimationOut) {
                ...
                console.log("===="), console.log("bbb"), console.log("====");
            }
            return el.removeClass("in").on("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", function(e) {
                if ("max-height" == e.originalEvent.propertyName) if (el.remove(), console.log("----"), 
                console.log(el), console.log("----"), "body" === opt.container) $("body").find("#page-alert .alert__wrap").length < 1 && $("body").removeClass("body-alert"); else if ("floating" === opt.container) ; else $("body").find("#page-alert .alert__wrap");
        ...
        var alertTimer, icon, clsBtn, clsName, defTemplate, opt = $.extend({}, {
            type: "primary",
            icon: "",
            title: "",
            message: "",
        ...
        clsName = opt.closeBtn ? " isclose" : "", defTemplate = '<div class="alert alert--' + opt.type + clsName + '" role="alert">' + clsBtn + '<div class="alert__body">', 
...
if (Dropdown.prototype.toggle = function(e) {
...
function(factory) {
    ...
    CreateMap.DEFAULTS = {
        ...
        that.options.sw = new kakao.maps.LatLng(that.options.swLatLng.Ma, that.options.swLatLng.La), 
        that.options.ne = new kakao.maps.LatLng(this.options.neLatLng.Ma, that.options.neLatLng.La), 
        console.log(that.options.swLatLng), that.options.rectangleBounds = new kakao.maps.LatLngBounds(that.options.sw, that.options.ne), 
    ...
    function modalRemotePlugin(option) {
        ...
        option.container ? $.ajax({
            ...
            success: function(data) {
                checkData = data.split("\n"), -1 < checkData.indexOf("<body>") && (data = /<body[\s\S]*?>([\s\S]*?)<\/body>/.exec(data)[1]);
    ...
    Modal.DEFAULTS = {
        ...
        this.$button.trigger(e), console.log(e), this.isShown || (this.isShown = !0, this.$body.addClass("modal__open"), 
