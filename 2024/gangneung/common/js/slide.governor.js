 ...
 * @constructor
 */
var SlideGov = function(selector, slideCount, slideControl, slideList, stepSize, animateSpeed, intervalTime, autoPlay, loop, direction, type, stepCount, thumbControl) {
	...
	this._thumbControl = thumbControl;
...
SlideGov.prototype = {
    ...
    resize: function() {
        ...
        if( this._type === 'horizontal' ) {
			...
			if( this._$slideList.css(this._basePosition) !== '0px' && this._$slideList.css(this._basePosition) !== 'auto' ) {
				this._$slideList.css("left", -this._stepSize);
			}
        } else if( this._type === 'vertical' ) {
			this._stepSize = this._$slideItem[0].offsetHeight;
			if( this._$slideList.css(this._basePosition) !== '0px' && this._$slideList.css(this._basePosition) !== 'auto' ) {
				this._$slideList.css("top", -this._stepSize);
			}
        }
    },
    ...
    _init: function() {
        ...
        this._orgLeft = parseInt(this._$slideList.css("left"),10);

        // 목록 전체 수
        this._slideTotal = parseInt((this._$slideItem.length / this._stepCount) + ((this._$slideItem.length % this._stepCount) > 0 ? 1 : 0 ), 10);
...
/*
			...
			if( this._$slideList.css(this._basePosition) === '0px' || this._$slideList.css(this._basePosition) === 'auto' ) {
				for( i = 0; i<this._stepCount; i++ ) {
					// 맨뒤 슬라이드 복사함
					var lastItem = this._$slideList.children().last().clone(true);
					// 만 앞으로 추가함
					lastItem.prependTo(this._$slideList);
					// 맨뒤 슬라이드 제거함
					this._$slideList.children().last().remove();
				}
				// 슬라이드 위치 정렬
				this._$slideList.css(this._basePosition, '-' + ((this._stepSize*this._stepCount) + 'px'));
            }
*/
            // 애니메이션 실행중으로 변경
            this._animated = true;
            // 슬라이드 위치 0으로
            this._slidePosition = -(this._slideNumber * this._stepSize);
            // 슬라이드 함!!!
            this._slidePlay();
...
*/
            ...
            this._animated = true;
            // 슬라이드 위치 조정함
            this._slidePosition = -(this._slideNumber * (this._stepSize*this._stepCount));
            // 슬라이드 함!!!
            this._slidePlay();
     ...
     */
    _slidePlay: function() {
        var _self = this, thumbList = $(".thumb_list li");
		...
		//this._displayControlButton();
		if( this._thumbControl ) {
			thumbList.find("i").show();
			thumbList.eq(this._slideNumber).find("i").hide();
			slideThumb(this._slideNumber+1);
		}
    },

	_displayControlButton: function() {
		if( this._slideNumber === 0 ) {
			this._$slideControl.find(this._slideControl.slideControlPrev).hide();
			this._$slideControl.find(this._slideControl.slideControlNext).show();
		} else if( this._slideNumber === (this._slideTotal-1) ) {
			this._$slideControl.find(this._slideControl.slideControlPrev).show();
			this._$slideControl.find(this._slideControl.slideControlNext).hide();
		} else {
			this._$slideControl.find(this._slideControl.slideControlPrev).show();
			this._$slideControl.find(this._slideControl.slideControlNext).show();
		}
     ...
     * @private
     */
    _displaySlideControlPlay: function(removeClass, addClass, title) {
        this._$slideControlPlay.removeClass(removeClass);
        this._$slideControlPlay.addClass(addClass);
        this._$slideControlPlay.attr('title', title);
    },
	...
	clickButton: function(idx) {
		...
		if( idx < this._slideNumber ) {
			this._slideNumber = idx;
            // 애니메이션 실행중으로 변경
            this._animated = true;
            // 슬라이드 위치 조정함
            this._slidePosition = -(this._slideNumber * (this._stepSize*this._stepCount));
