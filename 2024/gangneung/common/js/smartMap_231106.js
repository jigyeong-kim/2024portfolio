...
function changeMarkers() {
	...
	lifemarkers.forEach(function(element, index) {
		...
		kakao.maps.event.addListener(marker, 'click', function() {
			ListenerOverlay(index, lifemarkers[index].latlng, lifemarkers[index].latitude, lifemarkers[index].longtitude, lifemarkers[index].title, lifemarkers[index].address, lifemarkers[index].tel, lifemarkers[index].dong, lifemarkers[index].homepage, lifemarkers[index].detail, lifemarkers[index].cntrwkPd, lifemarkers[index].cnstrctr, lifemarkers[index].cntrInfo, lifemarkers[index].chargerDept);
		});
	});
	setMarkers(map);
}
...
function ListenerOverlay(index, LatLng, Latitude, Longtitude, Title, Address, Tel, Dong, Homepage, Detail, CntrwkPd, Cnstrctr, CntrInfo, ChargerDept ) { 
	...
	var iwcontent2 = '<div class="customoverlay"><div class="overlaybox">';
		...
		if(ChargerDept != ''){
			iwcontent2 += '<li class="info_item clearfix chargerDept"><em class="info_kind">담당부서</em><span class="info_text">' + ChargerDept + '</span></li>';
		}
		iwcontent2 += '</ul></div>';
		iwcontent2 += '</div>';
...
function CreateOverlay(latitude, longtitude, thisTitle, thisAddress, thisTel, thisDong, thisHomepage, thisDetail, thisCntrwkPd, thisCnstrctr, thisCntrInfo, thisChargerDept) {
	...
	var iwcontent = '<div class="customoverlay"><div class="overlaybox">';
		...
		if(thisChargerDept != ''){
			iwcontent += '<li class="info_item clearfix chargerDept"><em class="info_kind">담당부서</em><span class="info_text">' + thisChargerDept + '</span></li>';
		}
		iwcontent += '</ul></div>';
		iwcontent += '</div>';
...
$(document).on("click", ".category .listbox .itembtn", function() {
	...
	CreateOverlay(ThisLatitude, ThisLongtitude, ThisTitle, ThisAddress, ThisTel, ThisDong, ThisHomepage, ThisDetail, ThisCntrwkPd, ThisCnstrctr, ThisCntrInfo, ThisChargerDept);
	panTo(ThisLatitude, ThisLongtitude);
});
