...
!function(i) {
    ...
    e.prototype.initADA = function() {
        ...
        for (var s = e.currentSlide, n = s + e.options.slidesToShow; s < n; s++)
            e.$slides.eq(s).attr("tabindex", -1 /*0 > -11*/
            );
        e.activateADA()
    }
    ,
    ...
    e.prototype.lazyLoad = function() {
        ...
        if (!0 === s.options.centerMode ? !0 === s.options.infinite ? o = (t = s.currentSlide + (s.options.slidesToShow / 2 + 1)) + s.options.slidesToShow + 2 : (t = Math.max(0, s.currentSlide - (s.options.slidesToShow / 2 + 1)),
...
(function($) {
    ...
    $(function() {
        ...
        //lnb
        var $lnb = $header.find('.lnb'), $lnbShow = $lnb.find('.menu_show'), $lnbHide = $lnb.find('.menu_hide'), $lnbDepthItem = $lnb.find('.depth_item'), $lnbMenu = $lnb.find('.menu'), $lnbDepth2FirstChild = $lnbMenu.find('.depth2 > :first-child'),$lnbSpy = $lnb.find('.spy'), lnbHeight;
        ...
        // 추가 - logo, search 포커싱시 lnb 열려있으면 닫기
        $('#header .search_btn, #header .logo_anchor').on('focusin',function(event){
            if(mode === 'pc' && $html.hasClass('lnb_open')){
                $lnbMenu.height('');
                $html.removeClass('lnb_open');
                $lnbDepthItem.removeClass('active');
            };
        ...
        $window.on('screen:tablet screen:phone', function(event) {
            ...
            if($lnbSpy.length) {
                console.log($lnbSpy.parents('.depth_item'));
                $html.addClass('lnb_open');
                $lnbSpy.parents('.depth_item').addClass('active');
            }
        });
        ...
        //사이드
        var $side = $container.find('.side'), $sideDepthLI = $side.find('.depth_item'), $sideSpy = $side.find('.spy');
        ...
        $sideDepthLI.on('click', function(event) {
            ...
            if ($depthText.find(eventTarget).length || $depthText[0] === eventTarget) {
                ...
                if ($this.children('.depth').length) {
                    $this.toggleClass('active').siblings('.depth_item').removeClass('active');
                    $side.find('.depth1_item').each(function(){
                        if(!$(this).hasClass('active')) $(this).find('.depth_item').removeClass('active');
                    })
                    event.preventDefault();
                }
            }
