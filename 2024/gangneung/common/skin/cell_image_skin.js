...
$.extend(localSynap, (function() {
	var member = {
        ...
        parseCellImgData : function(key, dpi) {
			...
			lastColWidth = ((obj.w%member.cellPieceWidth)==0)?member.cellPieceWidth:obj.w%member.cellPieceWidth;
			lastRowHeight = ((obj.h%member.cellPieceHeight)==0)?member.cellPieceHeight:obj.h%member.cellPieceHeight;
		...
		getImagePath: function(pageNum){
            if (localSynap.jobId != undefined) {
                return member.convertServerUri + localSynap.objList[pageNum].path + '?dpi='+defaultDPI;
            } else {
		...
		createCellImgElement: function (row, col, width, height, colSize) {
			...
			var html = '<img id="piece' + row + '_' + col +'" name="piece' + row + '_' + col + '" style="width:' + width + 'px;height:'+ height + 'px;user-drag:none;user-select:none;-moz-user-select:none;-webkit-user-drag:none;-webkit-user-select:none;-ms-user-select:none;" src="" onerror="image_error(this,' + row + ',' + col + ')" onload="onLoadImg(this,'+ row + ',' + col + ',' + width + ',' + height +',' + colSize + ')" title="" alt="" />';
		...
		parseTextXmlWithKey: function (rowIdx, colIdx, width, height, pageIdx) {
            ...
            pdfSearch.loadTextXmlAsync(xmlUrl, pageIdx,
			                           ...
			                           function () {
	                                       ...
	                                       pageArea.append(pdfSearch.getSpanString(pageIdx, 8, 8));
			                           },
