...
function ISToolbarManager(id) {
  ...
  this.render=function() {
    ...
    if(ret!="") return "<div style=\""+(flow?"float:left;":"")+"padding:5px 5px 3px 5px;background-image:url("+this.iconPath+"grpbg.gif)\">" + ret + "</div>";
...
function ISButton(id, icon, text, width, height) {
  ...
  this.toHTML=function() {
    ...
    s[j++]="<table cellpadding=0 cellspacing=0 align='left' style='"+(isIE7?("width:"+this.width+"px;"):"")+"margin:0px'><tr><td unselectable='on' style='text-align:left;padding:0px;padding-right:0px;VERTICAL-ALIGN: top;margin-left:0;margin-right:1px;margin-bottom:1px;width:"+this.width+"px;height:"+this.height+"px;'>";
      ...
      for (var it in this.grps) {
        if(this.grps[it].render) {
          if(isIE7) {
            ...
            s[j++]="<div style='float:left;margin:2px;'>";
            s[j++]=this.grps[it].render();
            s[j++]="</div>";
          }
        }
