	...
	if(!name.equals("") || !address.equals("")) {
		...
		try{
			...
			*/
			...
			String sql = "INSERT INTO SIGNATURE (SIGNATURE_NO, NAME, ADDRESS, RDATE) VALUES (SIGNATURE_NO_SEQ.NEXTVAL,?,?,?)";        // sql 쿼리
			pstmt = conn.prepareStatement(sql);                          // prepareStatement에서 해당 sql을 미리 컴파일한다.
			pstmt.setString(1, name);
			pstmt.setString(2, address);
			pstmt.setString(3, DateUtil.getNowDateTime("yyyyMMddHHmmss"));
			...
			if(pstmt != null) try{pstmt.close();}catch(SQLException sqle){
				sqlSystem.out.println("Connection Exception occurred");
...
<script>
	...
	if(chk == "Y") {
		alert("참여해 주셔서 감사합니다.");
		location.href = "http://<%=domainURL%>/www/contents.do?key=20105&amp;complate=Y";
	} else {
		alert("처리중 오류가 있습니다.");
		location.href = "http://<%=domainURL%>/www/contents.do?key=20105";
	}
</script>
