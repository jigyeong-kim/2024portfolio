$(function (){
	...
	$('.menu_sub a').on('click',function(){
		...
		$('html,body').animate({scrollTop:$('.sub_cont').eq(here_num).offset().top-top_height},800);
	});
	...
	$(window).scroll(function(){
		...
		if(x>header_move.top-30){
			...
			for (var i=0;i<col_total;i++){
				if($('.sub_cont').eq(i).offset().top-470-$('.menu_sub').height()<$(window).scrollTop()){
					$('.tap_menu li').find('a').removeClass('on');
